...
const Home = () => {
    ...
    const Sponsor = ({company, image}) => (
        <div className="flex self-center justify-self-center flex-col items-center justify-between mt-8">
            ...
            <h4 className="text-base uppercase text-center mt-2">
                {company}</h4>
        </div>
    );
    ...
    return (
        <main id="home-page">
            <div id="home-image" className={`mx-auto static p-5 md:p-5 relative ${animate ? 'animate-fade-in-down' : ''}`}>
                {/* Welcome title */}
                <title id="home-title" className="flex flex-col place-items-center mx-auto inline-block mt-96 w-11/12 lg:mt-72 pt-2 z-30">
                    <Header title="ADA'S TEAM" homepage="true" subtitle="Promoting diversity in STEAM with an emphasis on technology."/> {/* Arrow */}
            ...
            <section id="sponsors-list"
                ...
                <Loader loading={
                    ...
                    <div className="mb-8 grid grid-cols-2 grid-rows-3 md:grid-cols-3  md:grid-rows-2 lg:my-10">
                        {
                        sponsors && sponsors.map(({company_name, image}) => (
